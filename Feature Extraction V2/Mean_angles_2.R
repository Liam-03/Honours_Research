...
angles_df <- mouse_data_completed %>%
  select(PIN, event_type, stage, x,y, scroll_position_x,scroll_position_y, click_count)%>% 
  group_by(PIN, stage) %>%
  mutate(
    lg_x1 = lag(x),
    lg_y1 = lag(y),
  ...
  mutate(x.1 = x - lg_x1,
         ...
         theta_rad = acos(pmax(pmin(d.prod / (f.norm * s.norm), 1), -1)),
...
angles_df_2 <- angles_df %>%
  group_by(PIN, stage, movement_count) %>%
  summarise(mean_angle = mean(theta_degr, na.rm = TRUE),
            median_angle = median(theta_degr, na.rm = TRUE))

angles_mean_median <- angles_df_2 %>%
  group_by(PIN) %>%
  summarise(median_mean_angle = median(mean_angle, na.rm = TRUE),
            median_median_angle = median(median_angle, na.rm = TRUE),
            mean_mean_angle = mean(mean_angle, na.rm = TRUE))
...
mean_angle_df_per_click_PIN <- angles_df %>%
  ...
  summarize(
    ...
    global_mean = sm_angles / total_observations
  ) %>%
  filter(is.finite(global_mean)) %>%
  select(PIN, global_mean)

angles_mean_median <- merge(angles_mean_median, mean_angle_df_per_click_PIN, by = "PIN")
