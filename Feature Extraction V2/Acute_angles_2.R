...
acute_angles_per_click_df <- acute_angles_df %>%
  group_by(PIN, stage, movement_count) %>%
  summarise(acute_angle_sum = sum(acute_angle))

acute_angles_mean_median <- acute_angles_per_click_df %>%
  group_by(PIN) %>%
  summarise(mean_acute_angles = mean(acute_angle_sum, na.rm = TRUE),
            median_acute_angles = median(acute_angle_sum, na.rm = TRUE))
